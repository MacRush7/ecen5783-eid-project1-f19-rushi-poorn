...
3.	The UI has an additional main status text box that provides critical system updates occurring over the execution cycle of the system. Plus, the UI has also has a database status line that reports the condition of the database to the user. For more detailed info into this additional features, please refer to the below sub-segments:
